...
Use this to find an entry's `id` — e.g. to spot and remove a duplicate with `delete_budget_entry`, or to patch an entry with `update_budget_entry`.

Optional `categoryId` filter (UUID, or `null` for uncategorized only).

### `update_budget_entry`

`POST /api/ai/tools/update_budget_entry`
...
All fields except `id` are optional (same shape as create: `amount`, `currency`, `date`, `categoryId`, `note`, `type`). Use `query_budget_entries` to find the `id` first.

### `query_budget_by_category`

`POST /api/ai/tools/query_budget_by_category`
...
Returns per-category totals for the month — useful for questions like "how much did I spend on Food?":
